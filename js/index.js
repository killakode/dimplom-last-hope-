document.addEventListener("DOMContentLoaded", () => {
    ...
    Request('POST', 'https://jscp-diplom.netoserver.ru/', 'event=update', function(response) {
        let data = {};
        ...
        data.films.forEach((film) => {
            ...
            data.halls.forEach((hall) => {
                let seances = data.seances.filter((seance) => ((seance.seance_hallid == hall.hall_id) && (seance.seance_filmid == filmId)));
                if (seances.length > 0) {
                    seancesHTML += `
        ...
        movieSeances.forEach((movieSeance) =>
            movieSeance.addEventListener("click", (event) => {
                ...
                localStorage.clear();
                localStorage.setItem("selectedSeanse", JSON.stringify(selectSeanse));
            })
        );

        let seancesUpdate = (timeStampDay) => {
            movieSeances.forEach((movieSeance) => {
        ...
        let linksForDay = Array.from(document.querySelectorAll(".page-nav__day"));

        linksForDay.forEach((dayLink) =>
            dayLink.addEventListener("click", (event) => {
                ...
                let timeStampDay = getTimeStampDay(event);
                seancesUpdate(timeStampDay);
            })
        );
